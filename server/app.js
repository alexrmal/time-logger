...
function createApp(options = {}) {
  ...
  app.post('/api/sessions/clock-out', (req, res) => {
    ...
    const sessionIndex = data.sessions.findIndex(
      (s) => s.id === session_id && !s.clock_out_time
    );
  ...
  app.get('/api/analytics', (req, res) => {
    ...
    const uniqueDays = new Set(
      sessions.map((s) => moment(s.clock_in_time).format('YYYY-MM-DD'))
    ).size;
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    app.get('*', (req, res) => {
      res.json({
        message:
          'Personal Efficiency Dashboard API - Frontend should be running on port 3000'
      });
    });
  }
